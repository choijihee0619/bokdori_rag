...
### 5) main.py의 역할과 통합 구조 예시

* 각 파트별(main.py)은 독립적으로 FastAPI 앱을 실행하고, API 개발/테스트에 활용
* 최종적으로 프로젝트 통합 단계에서는 루트(main.py)를 두고, 여러 서비스(챗봇, STT, TTS 등)를 하나로 통합하여 전체 서버 실행의 엔트리포인트로 사용
* 개별 서비스 main.py는 각자의 독립 개발/테스트 용도, 루트 main.py는 전체 시스템 실행 및 라우팅 역할

#### 예시 구조도

```
bokdori_ai_project/
├── main.py                # 전체 통합 실행(루트, 엔트리포인트)
├── rag_chatbot/
│     ├── main.py          # 챗봇 파트 독립 실행/개발용
│     └── ...
├── stt_service/
│     ├── main.py          # STT 파트 독립 실행/개발용
│     └── ...
├── tts_service/
│     ├── main.py          # TTS 파트 독립 실행/개발용
│     └── ...
└── frontend/
      └── ...
```
